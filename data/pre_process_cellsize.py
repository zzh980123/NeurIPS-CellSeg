...
def main():
    ...
    parser.add_argument("-o", '--output_path', default='./data/Train_Pre_cell_size', type=str, help='preprocessing data path')
    ...
    for img_name, gt_name in zip(tqdm(img_names), gt_names):
        ...
        avg_size = np.count_nonzero(gt_data) / gt_data.max()
        write2json(join(target_path, 'labels', gt_name.split('.')[0] + '.json'), size=avg_size)
