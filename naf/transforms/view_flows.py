...
if __name__ == '__main__':
    ...
    for f, ff in tqdm.tqdm(zip(files, flow_files), total=len(files)):
        ...
        #     instance = imageio.imread_v2(f)
        # else:
        #     instance = tifffile.imread(f)
        # print(f, np.unique(instance))
        # plot.imshow(instance)
        # plot.show()
        ...
        s = tifffile.imread(ff.replace(".png", "_flows.tif"))
        ...
        # plot.imshow(im)
        # plot.show()
        print(s.shape, s[:1].max(), im.shape)
        # plot.imshow([s[:1].transpose(1, 2, 0), s[1:2].transpose(1, 2, 0)])
        # plot.show()
        imgs = [s[:1].transpose(1, 2, 0), s[1:2].transpose(1, 2, 0), im]
