...
def main():
    ...
    for epoch in range(1, max_epochs):
        ...
        for step, batch_data in enumerate(train_loader, 1):
            ...
            outputs = torch.nn.functional.gumbel_softmax(outputs, dim=1, hard=True, tau=1 - epoch / max_epochs)

            diff_unccentmap = torch.prod(outputs, dim=1)
            lll_uncentmap = outputs[:, 1:2] * outputs[:, 2:] * (1 - outputs[:, 0:1])

            # lll_uncentmap = lll_uncentmap / lll_uncentmap.max() * 2
            # outputs[:, 2:] = outputs[:, 2:] + (lll_uncentmap * 27 / 2).detach()

            loss = loss_function(outputs, labels_onehot) + 5 * torch.mean(lll_uncentmap + diff_unccentmap)
            loss.backward()
            optimizer.step()
