...
from monai.networks.layers import DropPath, trunc_normal_, Conv, get_act_layer, get_norm_layer
from monai.utils import ensure_tuple_rep, optional_import

rearrange, _ = optional_import("einops", name="rearrange")


class SwinUNETRV3(nn.Module):
    """
...
class DeformableConvLayer(nn.Module):
    ...
    def __init__(self, shape, in_channel, out_channel, kernel_size, stride, spatial_dims=2, max_view=11):
        super().__init__()
        ...
        self.group_size = min(kernel_size) ** 2

        self.sp = Conv[Conv.CONV, spatial_dims](
            in_channels=in_channel, out_channels=in_channel * self.group_size, kernel_size=1, stride=1, padding=0
        )

        self.fusion = Conv[Conv.CONV, spatial_dims](
            in_channels=in_channel, out_channels=in_channel, kernel_size=1, stride=1, padding=0
        )
        ...
        self.trans = SpatialTransformer(shape, mode="nearest")
    ...
    def forward(self, x):
        ...
        offset = F.sigmoid(self.offset_) * self.max_view - self.max_view / 2
        B = x.shape[0]
        x_f_list = [x_f[:, i::self.group_size] for i in range(self.in_channel)]

        out = 0
        for f in x_f_list:
            outs = []
            for c in range(self.in_channel):
                transport = self.one_grid * offset[:, c].view((-1, ) + (1, ) * (x.ndim - 2))
                transport = torch.stack([transport] * B)
                outs.append(self.trans(f[:, c: c+1], transport))
            concat_ = torch.concat(outs, dim=1)
            out += self.fusion(concat_)

        return self.conv(out / self.in_channel)
...
class UnetBasicBlockDFC(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = DeformableConvLayer(
            shape, in_channels, out_channels, kernel_size, stride, spatial_dims, max_view=17
        )
